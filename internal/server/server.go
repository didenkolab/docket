...
type Options struct {
	...
	Template string
	// Programs lets this server run what the vault declares: its reactions, and
	// the pages and panels drawn from a program's output.
	//
...
type Server struct {
	...
	// programs says this server may run what the vault declares.
	programs bool
...
func (s *Server) Handler() http.Handler {
	...
	mux.HandleFunc("GET /views/new", s.handleViewEditForm)
	mux.HandleFunc("GET /views/edit/{path...}", s.handleViewEditForm)
	mux.HandleFunc("POST /views/save", s.handleViewSave)
